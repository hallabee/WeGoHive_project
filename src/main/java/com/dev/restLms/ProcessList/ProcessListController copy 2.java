...
//                     Optional<ProcessListCourse> findEnrollDate = processListCourseRepository.findBycourseId(courseId);
//                     Long nowDate = Long.parseLong(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss")));
//                     if(Long.parseLong(findEnrollDate.get().getEnrollEndDate()) < nowDate || Long.parseLong(findEnrollDate.get().getEnrollStartDate()) > nowDate){
